...
const MovieSelectionCard = ({ movie, isSelected, onToggle }) => {
  ...
  const posterUrl = movie.poster_path 
    ? `${process.env.REACT_APP_IMAGES}/${movie.poster_path}`
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-black bg-opacity-30 flex flex-col justify-end p-4">
          ...
          {/* Thông tin phim - THÊM String() ĐỂ AN TOÀN */}
          <div className="text-white">
            <h3 className="text-xl font-bold mt-2">{String(movie.title || 'No Title')}</h3>
            <p className="text-xs text-gray-300">{String(movie.genres || 'Unknown Genre')}</p>
            <div className="mt-1 text-sm flex items-center space-x-3">
              <span className="text-yellow-400">⭐ {String(movie.rating || '0')}</span>
              <span className="text-gray-400">| {String(movie.votes || '0')}</span>
            </div>
          </div>
        </div>
      </div>
