  ...
  return (
    <header className="absolute top-0 left-0 w-full z-50 flex justify-between items-center px-8 py-4 bg-transparent">
      ...
      <nav className="hidden md:flex bg-black/40 backdrop-blur-none px-8 py-2 rounded-full space-x-6 text-white text-sm">
        ...
        <Link to="/" className="hover:text-rose-700 transition">Movies</Link>
        <span className="hover:text-rose-700 transition cursor-pointer">Theatres</span>
        <span className="hover:text-rose-700 transition cursor-pointer">Releases</span>
      </nav>
        ...
        {user ? (
            <div className="flex items-center gap-3 animate-in fade-in duration-300">
                <div className="hidden md:flex flex-col items-end mr-2">
                    <span className="text-white text-sm font-bold leading-none">{user.fullname}</span>
                    <span className="text-[10px] text-gray-400 uppercase tracking-wider">{user.role || 'Member'}</span>
                </div>
                
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-rose-600 to-purple-600 p-[1px]">
                    <img 
