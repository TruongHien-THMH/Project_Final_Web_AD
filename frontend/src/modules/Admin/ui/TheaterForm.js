...
export default function TheaterForm({ onCreated }) {
    ...
    return (
        <div className="bg-gray-800 p-6 rounded-3xl shadow-xl border border-gray-700 h-full">
            <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-bold text-white">Create New Theater</h2>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label className="text-gray-400 text-sm mb-1 block">Theater Name</label>
                    <input 
                        type="text" placeholder="e.g. CGV Aeon Mall" className={inputClass}
                        value={formData.iName} onChange={e => setFormData({...formData, iName: e.target.value})}
                        required
                    />
                </div>
            ...
            {/* Responsive Grid: 1 cột mobile, 2 cột desktop */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="text-gray-400 text-sm mb-1 block">Total Halls</label>
                        <input 
                ...
                className="w-full mt-4 bg-gradient-to-r from-rose-600 to-rose-500 text-white font-bold py-3 rounded-xl hover:shadow-lg hover:shadow-rose-900/40 transition-all active:scale-95 flex items-center justify-center gap-2"
                >
                {loading ? <><Loader2 className="animate-spin" size={20}/> Creating...</> : 'Create System'}
                </button>
            </form>
        </div>
    );
